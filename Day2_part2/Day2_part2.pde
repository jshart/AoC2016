...
String filebase = new String("C:\\Users\\jsh27\\OneDrive\\Documents\\GitHub\\AoC2016\\Day2_part2\\data\\mydata");
...
void setup() {
  ...
  {
    output+=" "+kp.walk(input.lines.get(i));
    println(">"+output);
  }
  println("["+output+"]");
}
...
public class Keypad
{
  String[] pad={"00100","02340","56789","0ABC0","00D00"};
  int x=0,y=2;
  ...
  public char walk(String s)
  {
    ...
    for (int i=0;i<s.length();i++)
    {
      ...
      {
        case 'U':
          y=(movePermitted(0,-1)==true?y-1:y);
          break;
        case 'D':
          y=(movePermitted(0,+1)==true?y+1:y);
          break;
        case 'R':
          x=(movePermitted(+1,0)==true?x+1:x);
          break;
        case 'L':
          x=(movePermitted(-1,0)==true?x-1:x);
          break;
  ...
  public boolean movePermitted(int xd, int yd)
  {
    int newx=x+xd;
    int newy=y+yd;
    ...
    // valid location inside grid?
    if (pad[newy].charAt(newx)=='0')
    {
      return(false);
    }
