...
String filebase = new String("C:\\Users\\jsh27\\OneDrive\\Documents\\GitHub\\AoC2016\\Day3_part2\\data\\mydata");
...
void setup() {
  ...
  IntList L0 = new IntList();
  IntList L1 = new IntList();
  IntList L2 = new IntList();

  // Loop through each input item... and append the 3 fields to 3 different lists
  for (i=0;i<input.lines.size();i++)
  {
    strings=input.lines.get(i).split(",");
    
    L0.append(Integer.parseInt(strings[0]));
    L1.append(Integer.parseInt(strings[1]));
    L2.append(Integer.parseInt(strings[2]));
  }
  
  // join all of the lists together end to end
  L0.append(L1);
  L0.append(L2);
  

  // Loop through the combined list, selecting chunks of 3 at a time
  for (int k=0;k<L0.size();k++)
  {
    ...
    // Convert the parts to ints and find the largest
    // component as we go
    for (j=0;j<3;j++)
    {
      m.set(nums[j]);
    ...
    resultString="";
    foundOne=false;
    // Add up components that are *not* the largest 
    for (j=0;j<3;j++)
    {
      if (nums[j]!=m.value || foundOne==true)
      {
        resultString+=nums[j]+",";
        total+=nums[j];
      }
    ...
    // if the total of the 2 smaller sides are larger than the max value
    // then its a valid corridor
    if (total>m.value)
    {
      validTriangles++;
      resultString+=" {VALID}";
  ...
  println("valid;"+validTriangles);
  println("invalid;"+invalidTriangles);
  println("Total:"+(validTriangles+invalidTriangles));
}
