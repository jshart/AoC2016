...
for i,l in enumerate(lines):
    ...
    if compareCheckSum==checkSum[i]:
        ...
        # rotation key is the mod of the checksum by 26 (length of the alphabet)
        rotationKey=int(sectorCode[i])%26
        decodedKey=""

        if debug:
            masterLine[i]="qzmt-zixmtkozy-ivhz-343"
            rotationKey=343%26
        for c in masterLine[i]:
            if c=='-':
                decodedKey+=' '
            elif c.isalpha():
                o = ord(c)+rotationKey
                # if we're passed the ASCII code for 'z', we need to loop the index
                # back to the start
                if o>=122:
                    o=ord('a')+(o-123)
                decodedKey+=chr(o)
